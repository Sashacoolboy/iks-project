...
test('indexAdapter: 1027 entries, gold mapping, nist reverse index', () => {
  const idx = indexAdapter(adapter);
  assert.equal(idx.byAssessmentId.size, 1027);
  ...
  assert.equal(idx.nistVerified.size, 7); // 1+1+3+1 (AC-02) + 1 (AU-11)
});
